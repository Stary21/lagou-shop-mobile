<template>
  <!-- 导航 -->
  <van-nav-bar title="订单确认" left-text="返回" left-arrow />
  <!-- 主体内容 -->
  <div class="container">
    <!-- 1 地址区域 -->
    ...
    <van-popup v-model:show="popupStatus" position="bottom">
      <van-address-list v-model="chosenAddressId" :list="addressList" default-tag-text="默认" @click-item="changeAddress"
  ...
  <!-- 提交订单区域 -->
  <van-submit-bar :price="totalPrice" label="订单总结：" button-text="立即付款" @submit="showPayPanel = true" />
  <!-- 弹出付款区域 -->
  <van-action-sheet v-model:show="showPayPanel" title="请选择支付方式" cancel-text="取消" close-on-click-action>
    <!-- 付款列表区域 -->
    <template #default>
      <van-radio-group v-model="paymentMethod">
        <van-cell-group>
          <!-- 余额支付 -->
          <van-cell clickable title="微信支付" label="微信快捷支付" @click="paymentMethod = 'wechat'" size="large" center>
            <template #icon>
              <van-icon name="http://www.lgstatic.com/lg-app-fed/pay/images/wechat_b787e2f4.png" size="30"
          ...
          <!-- 支付宝 -->
          <van-cell clickable title="支付宝" label="支付宝快捷支付" @click="paymentMethod = 'alipay'" size="large" center>
            <template #icon>
              <van-icon name="http://www.lgstatic.com/lg-app-fed/pay/images/ali_ed78fdae.png" size="30"
                :style="{ marginRight: '12px' }" />
            </template>
            <template #right-icon>
          ...
          <!-- 余额支付 -->
          <van-cell clickable title="余额支付" :label="`可用余额为：${yue}元`" @click="paymentMethod = 'yue'" size="large" center>
            <template #icon>
              <van-icon name="gold-coin" color="#FF9900" size="30" :style="{ marginRight: '12px' }" />
            </template>
            <template #right-icon>
          ...
          <van-cell>
            <van-button round type="danger" block @click="handlePay">
              去支付
            </van-button>
          </van-cell>
        </van-cell-group>
